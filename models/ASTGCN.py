...
def cheb_polynomial_torch(L_tilde, K):   # 使用pytorch张量计算切比雪夫多项式
    N = L_tilde.shape[0]
...
class ChebConvWithSAt(nn.Module):
    ...
    def __init__(self, device, k, fusiongraph, in_channels, out_channels):
        """
        Args:
            k(int): K-order
    ...
    def forward(self, x, spatial_attention):
        ...
        cheb_polynomials = cheb_polynomial_torch(L_tilde, self.K)  # 计算切比雪夫多项式
...
class ASTGCNBlock(nn.Module):
    def __init__(self, device, fusiongraph, in_channels, k, nb_chev_filter,
    ...
    #              time_strides, cheb_polynomials, num_of_vertices, num_of_timesteps):
        ...
        self.cheb_conv_SAt = ChebConvWithSAt(device, k, fusiongraph, in_channels, nb_chev_filter)
...
class ASTGCN_Submodule(nn.Module):
    ...
    #              time_strides, cheb_polynomials, output_window, output_dim, num_of_vertices):
        ...
        self.BlockList = nn.ModuleList([ASTGCNBlock(device, fusiongraph, in_channels, k, nb_chev_filter,
                                                    nb_time_filter, time_strides,
                                                    num_of_vertices, time_strides * output_window)])

        self.BlockList.extend([ASTGCNBlock(device, fusiongraph, nb_time_filter, k, nb_chev_filter,
                                           nb_time_filter, 1, num_of_vertices, output_window) for _ in range(nb_block-1)])
